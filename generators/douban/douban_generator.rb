class DoubanGenerator < Rails::Generator::Base
  def manifest  
    record do |m|  
      ...
      m.routh :douban_login,:controller => :douban,:action => :new
      m.routh :douban_callback,:controller => :douban,:action => :callback
      m.routh :douban_logout,:controller => :douban,:action => :destroy
    end  
  end
end
